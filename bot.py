...
async def remove(ctx, base: str):
    ...
    await ctx.send(f"🗑 **{base}** removed from tracking.")
...
async def generate_report(channel):
    ...
    for entry in get_all_bases():
        ...
        if remaining <= 0:
            delete_base(entry["user_id"], entry["base_name"])
            continue

        # Fetch member from this guild for server nickname
        member = channel.guild.get_member(int(entry["user_id"]))
    ...
    lines = ["📅 **Dune Power Report (Lowest → Highest):**\n"]
    for item in report_data:
        ...
        if remaining <= 1440:
            ...
            emoji = "🟢"
        lines.append(f"{emoji} **{item['base_name']}** ({item['user']}) → {format_minutes(remaining)}")

    await channel.send("\n".join(lines))
    print("[Tracker] Report sent.")
